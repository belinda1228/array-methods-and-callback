...
const postTaxPrices = prices.map(aftertax=> prices*1.07);
...
// CODE HERE
const totalPopulation = populations.reduce(function(total,cur){
  return total+cur
})
...
const myStrongest = monstersInYourPocket.filter(function(monsters){
  return monsters["cp"]>200;
}
/* Provide Your Callback Here */)
...
// CODE HERE
const totalOrders = orders.map(total =>order.price+(order.price*order.tax))
const totalOrders = orders.map(function(total){
  return orders.price +orders.price*order.tax
})
...
// CODE HERE
let bobsTotal = 0
const findBob = purchases.filter(function(purchase){
  return purchases.owner === "Bob"
}).reduce(function (sum,purchases){
